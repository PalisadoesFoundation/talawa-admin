...
class TestTranslationCheck(unittest.TestCase):
    ...
    def test_get_keys_recursion(self):
        ...
        self.assertEqual(translation_check.get_translation_keys({"a": {"b": "c"}}), {"a.b"})
    ...
    def test_find_tags_string(self):
        ...
        self.assertEqual(translation_check.find_translation_tags("t('key')"), {"key"})
    ...
    def test_find_tags_namespace(self):
        ...
        self.assertEqual(translation_check.find_translation_tags("t('common:key')"), {"key"})
    ...
    def test_find_tags_dynamic_ignored(self):
        ...
        self.assertEqual(len(translation_check.find_translation_tags("t(var)")), 0)
    ...
    def test_get_files_directory(self):
        ...
        src.mkdir()
        (src / "app.tsx").touch()
        self.assertEqual(len(translation_check.get_target_files(None, [str(src)], [".tsx"])), 1)
    ...
    def test_get_files_exclude_spec(self):
        ...
        self.assertEqual(len(translation_check.get_target_files(None, [str(src)], [".tsx"])), 0)

    def test_get_files_explicit(self):
        """Verify explicit file selection."""
        f = os.path.join(self.test_dir, "f.ts")
        Path(f).touch()
        self.assertEqual(len(translation_check.get_target_files([f], None, [".ts"])), 1)

    def test_main_success_flow(self):
        """Verify exit code 0 on successful validation."""
        with patch("sys.argv", ["translation_check.py", "--locales-dir", self.en_dir, "--directories", self.test_dir]):
    ...
    def test_main_missing_flow(self):
        ...
        with patch("sys.argv", ["translation_check.py", "--locales-dir", self.en_dir, "--files", str(p)]):
            with patch("sys.exit") as mock_exit:
                translation_check.main()
    ...
    def test_main_error_flow(self):
        ...
        with patch("sys.argv", ["translation_check.py", "--locales-dir", "/invalid"]):
            with patch("sys.exit") as mock_exit:
                translation_check.main()
    ...
    def test_entry_point_subprocess(self):
        ...
        script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "translation_check.py")
        result = subprocess.run(
            [sys.executable, script_path, "--locales-dir", self.en_dir, "--directories", self.test_dir],
            capture_output=True,
            text=True,
            check=False
        )
        self.assertEqual(result.returncode, 0)
        self.assertIn("All translation tags validated successfully", result.stdout)
