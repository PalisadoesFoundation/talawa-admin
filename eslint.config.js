  ...
  // Test file-specific configuration for mock isolation
  {
    ...
    rules: {
      ...
      'no-restricted-syntax': [
        'error',
        {
          selector: 'Literal[value=/202[3-9]-\\d{2}-\\d{2}/]',
          message:
            'Avoid hardcoded date strings in tests. Use dynamic dates with dayjs instead (e.g., dayjs().add(30, "days").format("YYYY-MM-DD")).',
        },
        {
          selector:
            'Literal[value=/\\d{1,2}\\s+(January|February|March|April|May|June|July|August|September|October|November|December)\\s+202[3-9]/]',
          message:
