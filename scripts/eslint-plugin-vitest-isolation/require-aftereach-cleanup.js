...
module.exports = {
  ...
  create(context) {
    ...
    return {
      ...
      'CallExpression[callee.name="afterEach"]'(node) {
        ...
        if (
          ...
          if (bodyNode.type === 'BlockStatement') {
            ...
            if (
              cleanupMethods.clearAllMocks ||
              cleanupMethods.restoreAllMocks ||
              cleanupMethods.resetModules
            ) {
              hasCleanup = true;
            }
      ...
      'Program:exit'(node) {
        const hasMocks =
          mockUsage.hasFn || mockUsage.hasMock || mockUsage.hasSpyOn;
              ...
              if (firstDescribe && firstDescribe.arguments[1]) {
                ...
                if (callback.body && callback.body.type === 'BlockStatement') {
                  ...
                  if (describeBody.length > 0) {
                    ...
                    const leadingText = sourceCode.text.substring(
                      lineStart,
                      stmtStart,
                    );
                    ...
                    const descLineStart = sourceCode.getIndexFromLoc({
                      line: callback.body.loc.start.line + 1,
                      column: 0,
                    });
                    const nextLineStart =
                    ...
                    if (
                      ...
                      const leadingMatch = sourceCode.text
