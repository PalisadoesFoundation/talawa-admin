...
const mockT = (key: string, _options: Record<string, unknown> = {}): string => {
  ...
  if (key === 'recurrence.weeklyOn') return `Weekly on {{day}}`;
  if (key === 'recurrence.monthlyOn') return `Monthly on day {{day}}`;
  if (key === 'recurrence.annuallyOn') return `Annually on {{month}} {{day}}`;
  if (key === 'recurrence.everyWeekday')
    return 'Every weekday (Monday to Friday)';
  if (key === 'recurrence.custom') return 'Custom...';
  if (key === 'selectRecurrencePattern') return 'Select recurrence pattern';
  return key;
};
