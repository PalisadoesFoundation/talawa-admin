...
test('should handle missing community data', async () => {
  ...
  const hasSessionTimeout = timeoutCalls.some((call: Parameters<typeof setTimeout>) => {
    const [_, ms] = call;
...
test('should handle session timeout data updates', async () => {
  ...
  const hasWarningTimeout = timeoutCalls.some(call => {
    ...
    return Math.abs(duration - expectedWarningTime) <= expectedWarningTime * 0.05; // ±5%
  });

  const hasSessionTimeout = timeoutCalls.some(call => {
    const duration = call[1] as number;
    return Math.abs(duration - expectedSessionTime) <= expectedSessionTime * 0.05; // ±5%
  });
...
// test('should handle errors during session extension', async () => {
//   const consoleErrorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
...
//   await vi.waitFor(() => {
//     // Replace 'isSessionActive' with appropriate logic or property
//     expect(result.current.getSessionState()).toBe('active'); // Example replacement
//   });

//   // Mock setTimeout to throw an error
//   const originalSetTimeout = global.setTimeout;
//   global.setTimeout = vi.fn(() => {
//     throw new Error('Timer setup error');
//   }) as unknown as typeof setTimeout;

//   try {
//     // Trigger extendSession
//     await result.current.extendSession();
//   } catch {
//     // Ignore errors thrown directly in the test
//   }
