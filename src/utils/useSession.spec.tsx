...
describe('useSession Hook', () => {
  ...
  test('should handle visibility change to visible', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(mockNotificationToast.warning).toHaveBeenCalledWith(
        'sessionWarning',
        expect.any(Object),
      );
    });
  ...
  test('should handle visibility change to hidden and ensure no warning appears in 15 minutes', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(mockNotificationToast.warning).not.toHaveBeenCalled();
    });
  ...
  test('should call handleLogout after session timeout', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(mockNotificationToast.warning).toHaveBeenCalledTimes(2);

      expect(mockNotificationToast.warning).toHaveBeenNthCalledWith(
        1,
        'sessionWarning',
        expect.any(Object),
      );
