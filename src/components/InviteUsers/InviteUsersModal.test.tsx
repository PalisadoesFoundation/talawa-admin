...
const mockInviteUserMutationSuccess = {
  request: {
    query: INVITE_USER,
    variables: { userId: 'user1', eventId: 'event123' },
  },
  result: {
    data: {
      inviteUser: { _id: 'user1' },
    },
  },
};

const mockInviteUserMutationFailure = {
  request: {
    query: INVITE_USER,
    variables: { userId: 'user1', eventId: 'event123' },
  },
...
describe('InviteUserModal Component', () => {
  test('renders modal and adds attendee successfully', async () => {
    const { getByText, getByLabelText } = render(
      <MockedProvider
        mocks={[
          mockEventAttendeeQuery,
          mockMembersListQuery,
          mockInviteUserMutationSuccess,
        ]}
      >
        <BrowserRouter>
          <Provider store={store}>
