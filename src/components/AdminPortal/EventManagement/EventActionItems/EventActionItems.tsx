...
import type { InterfaceGetEventActionItemsQuery } from 'utils/interfaces';
...
const EventActionItems: React.FC<InterfaceEventActionItemsProps> = ({
  ...
  const statusModal = useModalState(false);
  ...
  const handleModalClick = useCallback(
    ...
    [sameModal, deleteModal, viewModal, statusModal],
  );
  ...
  useEffect(() => {
    if (eventData && eventData.event) {
      const items = eventData.event.actionItems.edges.map(
        (edge) => edge.node as IActionItemInfo,
      );
