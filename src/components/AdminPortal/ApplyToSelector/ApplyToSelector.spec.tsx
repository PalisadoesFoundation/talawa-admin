...
import type { ApplyToType } from './ApplyToSelector';
...
describe('ApplyToSelector', () => {
  ...
  it('calls onChange with series when series radio is clicked', () => {
    const onChange = vi.fn();
    renderComponent('instance', onChange);

    fireEvent.click(screen.getByLabelText(/entire series/i));

    expect(onChange).toHaveBeenCalledWith('series');
  });

  it('calls onChange with instance when instance radio is clicked', () => {
    const onChange = vi.fn();
    renderComponent('series', onChange);

    fireEvent.click(screen.getByLabelText(/this event only/i));
