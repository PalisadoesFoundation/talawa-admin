...
> = ({ show, onClose, onConfirm, plugin }) => {
   ...
   * Prevents closing if an action is in progress.
   */
  const handleClose = () => {
    if (isLoading) {
      return;
  ...
  return (
    <BaseModal
      show={show}
      // FIX: Use our wrapper function that checks isLoading
      onHide={handleClose}
      // Prevent closing via backdrop click when loading
      backdrop={isLoading ? 'static' : true}
      // Prevent closing via ESC key when loading
      keyboard={!isLoading}
      title={t('pluginStore.uninstallPlugin')}
      footer={
        <>
          <Button
            variant="secondary"
    ...
    >
      <div data-testid="uninstall-modal">
        <p className="mb-2" data-testid="uninstall-modal-title">
          {t('pluginStore.uninstallPluginMsg')}
        </p>
        <p className="text-secondary small">
          {t('pluginStore.uninstallWarning')}
        </p>
      </div>
    </BaseModal>
  );
