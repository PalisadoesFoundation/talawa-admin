...
import { useParams } from 'react-router-dom'; // Added for orgId
import { toast } from 'react-toastify';
import AboutImg from 'assets/images/defaultImg.png';
// Removed: import convertToBase64 from 'utils/convertToBase64';
...
import { useMinioUpload } from 'utils/MinioUpload'; // Added for MinIO upload
...
export default function OrgPostCard({
  ...
  const { orgId } = useParams<{ orgId: string }>(); // Added for MinIO
  const { uploadFileToMinio } = useMinioUpload(); // Added for MinIO
  ...
  // Modified to use MinIO instead of base64
      ...
      try {
        ...
        setPostFormState((prev) => ({
          ...
          attachments: [
            ...prev.attachments,
            {
              url: previewUrl, // Use local preview URL instead of base64
              mimeType: file.type,
              objectName, // Store MinIO objectName
            },
          ],
        }));
      } catch (error) {
  ...
  const updatePost = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      // Modified to use objectName instead of base64 data
      const attachmentsForUpdate = postFormState.attachments.map(
        (attachment) => ({
      ...
      const { data } = await updatePostMutation({
        variables: {
          input: {
            ...
            attachments: attachmentsForUpdate, // Send objectName references, not base64
          },
        },
      });
  ...
  // Helper function to get media URL from objectName
  const getMediaUrl = (objectName?: string, type?: string): string => {
    if (!objectName) return '';

    const endpoint = type?.startsWith('image/') ? 'images' : 'videos';
    return `/api/${endpoint}/${objectName}`;
  };
