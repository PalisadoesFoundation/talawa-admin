...
describe('PinnedPostCard Component Tests', () => {
  ...
  it('should open modal when pinned post card is clicked', async () => {
            ...
            organization: {
              ...
              posts: {
                edges: [
                  {
                    node: {
                      id: 'pinned-modal-test',
                      caption: 'Pinned Post for Modal',
                      creator: {
                        id: 'u1',
                        name: 'User1',
                        avatarURL: null,
    ...
    // Wait for pinned post to render
    await waitFor(
      () => {
        expect(screen.getByText('Pinned Post for Modal')).toBeInTheDocument();
      },
      { timeout: 5000 },
    );

    // Verify the pinned posts section is visible
    expect(screen.getByText(/pinned posts/i)).toBeInTheDocument();
  });
