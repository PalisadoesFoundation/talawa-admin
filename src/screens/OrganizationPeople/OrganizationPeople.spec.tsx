...
describe('OrganizationPeople', () => {
  ...
  test('handles avatar rendering for users without images', async () => {
    ...
    // Should render avatar component instead of image
    await waitFor(() => {
  ...
  test('handles navigation with missing cursors gracefully', async () => {
    const mockWithoutCursors = createMemberConnectionMock(
      {
        orgId: 'orgid',
        first: 10,
        after: null,
    ...
    const link = new StaticMockLink([mockWithoutCursors], true);
    ...
    // Try to navigate to next page (should not work due to missing cursors)
    const nextPageButton = screen.getByRole('button', { name: /next page/i });
    fireEvent.click(nextPageButton);
  ...
  test('toggles remove modal correctly', async () => {
    const mocks = [
      createMemberConnectionMock({
        orgId: 'orgid',
        first: 10,
    ...
    // Click outside or close the modal
    const modal = screen.getByTestId('removeMemberModal');
  ...
  test('handles member creation date formatting correctly', async () => {
    const mockWithDifferentDates = createMemberConnectionMock(
      {
        orgId: 'orgid',
        first: 10,
        after: null,
        ...
        edges: [
          {
            node: {
              ...
              emailAddress: 'john@example.com',
              avatarURL: 'https://example.com/avatar1.jpg',
              createdAt: '', // This should fall back to current date
            },
            cursor: 'cursor1',
          },
        ],
    ...
    const link = new StaticMockLink([mockWithDifferentDates], true);
    ...
    // Should display a formatted date (current date since createdAt is null)
