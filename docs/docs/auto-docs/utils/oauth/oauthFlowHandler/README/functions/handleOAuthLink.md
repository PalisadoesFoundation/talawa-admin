...
> **handleOAuthLink**(`client`, `provider`, `authorizationCode`, `redirectUri`): `Promise`\<[`InterfaceOAuthLinkResponse`](types\Auth\auth\README\interfaces\InterfaceOAuthLinkResponse.md)\>
...
`Promise`\<[`InterfaceOAuthLinkResponse`](types\Auth\auth\README\interfaces\InterfaceOAuthLinkResponse.md)\>
