...
1. If you've added code, then test suites must be added. 
   ...
   1. **_Linting and Formatting_:**
      1. The pull request must contain only properly linted and formatted code. You can use the scripts `yarn lint:fix` and `yarn format:fix` to do so automatically. You can read about the same in more detail in the [PR guidelines here](./PR-guidelines.md).
   2. **_Testing_:**
